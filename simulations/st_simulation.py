...
class Simulation:
    ...
    def run(self):
        ...
        if self.multi_proc:
            ...
            a = [ordered_results[i][1]["risk"] for i in range(num_processes)]
    ...
    def generate_plots(self):
        ...
        if "risk_std" in self.sim_config.plots:
            fig = plt.figure()
            plt.plot(x_axis, self.baseline_metrics["risk_std"], 'b', x_axis, self.baseline_metrics["kd_std"], 'r')
...
if __name__ == '__main__':
    ...
    example_2params_student.dest_dir = None
    sim = Simulation(example_2params_student,multi_proc=True)
    sim.run()
    plt.show()
