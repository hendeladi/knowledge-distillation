...
if __name__ == '__main__':
    logging.basicConfig(filename="Log_"+datetime.now().strftime("%d_%m_%Y__%H_%M_%S"),
                        filemode='a',
                        format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s',
                        datefmt='%H:%M:%S',
                        level=logging.DEBUG)
    a = simulations.configs.example_theory_check8

    sim = Simulation(a, multi_proc=True)
    print(sim.sim_config)
    logging.info(sim.sim_config.__str__())
    sim.run()
    #plt.show()
